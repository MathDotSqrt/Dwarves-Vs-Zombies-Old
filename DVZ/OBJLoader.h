...
#include <string> // String - STD String Library
#include <fstream> // fStream - STD File I/O Library
#include <math.h> // Math.h - STD math Library
		...
		bool LoadFile(std::string Path)
		{
			...
			while (std::getline(file, curline))
			{
				...
				if ((outputIndicator = ((outputIndicator + 1) % outputEveryNth)) == 1)
				{
					if (!meshname.empty())
					{
						LOG_LOADER("\r-%s\t| vertices > %zu\t| texcoords > %zu\t| normals > %zu\t| triangles > %I64u\t| material: %s",
							meshname.c_str(),
							Positions.size(),
							TCoords.size(),
							Normals.size(),
							Vertices.size() / 3,
							!MeshMatNames.empty() ? MeshMatNames.back().c_str() : "none");
						//std::cout
						//	<< "\r- " << meshname
