...
public:
	...
	PackedFreeList(size_t max_objects) {
	...
	PackedFreeList(PackedFreeList&& other) : PackedFreeList() {
		swap(other);
	}

	PackedFreeList& operator=(PackedFreeList&& other) {
		if (this != &other)
		{
			swap(other);
		}
...
template<class T>
typename PackedFreeList<T>::iterator begin(const PackedFreeList<T>& fl) {
	return fl.begin();
}

template<class T>
typename PackedFreeList<T>::iterator end(const PackedFreeList<T>& fl) {
	return fl.end();
}
